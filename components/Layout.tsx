...
// Fix: Added missing import for parseDateTime
...
export const Layout = ({ children }: React.PropsWithChildren) => {
  ...
  const notifications = useMemo(() => {
    ...
    ctes.forEach(c => {
        ...
        if (c.status === 'EM BUSCA') {
            list.push({ 
                ...
                type: 'BUSCA', 
                date: null, 
                text: `MERCADORIA EM BUSCA: CTE ${c.cte} série ${c.serie}`, 
                cteId: c.id, 
                sortDate: now + 5000,
            ...
            list.push({ 
                ...
                type: 'CRITICO', 
                date: null, 
                text: `CTE ${c.cte} está com prazo CRÍTICO!`, 
                cteId: c.id, 
                sortDate: now + 1000,
    ...
    // Novas Notas
    const recentNotes = [...notes].sort((a,b) => parseDateTime(b.date) - parseDateTime(a.date)).slice(0, 10);
    ...
    return list.sort((a, b) => b.sortDate - a.sortDate).slice(0, 30);
          ...
          <div className="fixed top-20 right-6 z-[9999] animate-slide-in-right cursor-pointer group" onClick={() => handleNotificationClick(latestNotification.cteId, latestNotification.id)}>
              <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 p-5 max-w-sm flex items-start gap-4 ring-2 ring-primary/5 hover:scale-[1.02] transition">
                  ...
                  <div className="flex-1">
                      ...
                      <p className="text-[10px] text-primary mt-2 font-bold underline">Clique para ver detalhes</p>
                  </div>
      ...
      {alertActive && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-red-900/95 backdrop-blur-md animate-fade-in">
          <div className="bg-white p-10 rounded-3xl shadow-2xl max-w-lg text-center border-t-8 border-red-600 animate-bounce-in relative overflow-hidden">
             ...
             <p className="text-gray-700 mb-8 font-semibold leading-relaxed">ALERTA CRÍTICO: Uma mercadoria que você deve acompanhar foi marcada como perdida.</p>
             ...
             <div className="flex gap-4 justify-center">
                ...
                <button onClick={openAlertCTE} className="flex-[2] bg-red-600 hover:bg-red-700 text-white font-black py-4 px-8 rounded-2xl shadow-xl transition transform hover:scale-105 flex items-center justify-center gap-3"><i className="ph-bold ph-magnifying-glass-plus"></i> RESOLVER AGORA</button>
             </div>
          </div>
        </div>
      )}
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden bg-[#F8F9FC]">
        <header className="h-20 flex items-center justify-between px-6 md:px-10 pt-6 pb-4 shrink-0 z-30">
           <div className="flex items-center gap-4">
             ...
             <h2 className="text-2xl font-bold text-gray-800 hidden md:block tracking-tight">{menuItems.find(i => i.path === location.pathname)?.label || 'Bem-vindo'}</h2>
           </div>
           ...
           <div className="flex items-center gap-6">
               ...
               {isNotifOpen && (
                 <div className="absolute right-0 top-12 w-80 md:w-96 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden animate-fade-in origin-top-right z-50">
                    <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                        <div className="flex items-center gap-2"><h3 className="font-bold text-gray-800">Notificações</h3></div>
                        {unreadCount > 0 && <button onClick={() => { const allIds = notifications.map(n => n.id); setReadNotificationIds(prev => Array.from(new Set([...prev, ...allIds]))); }} className="text-[10px] font-bold text-primary hover:underline">Limpar</button>}
                    </div>
                    <div className="max-h-[60vh] overflow-y-auto">
                        {notifications.length === 0 ? (
                            <div className="p-8 text-center text-gray-400 flex flex-col items-center"><i className="ph-duotone ph-bell-slash text-3xl mb-2 opacity-50"></i><p className="text-sm">Vazio.</p></div>
                        ) : (
                                    ...
                                    return (
                                        <div key={idx} onClick={() => handleNotificationClick(item.cteId, item.id)} className={`p-4 hover:bg-indigo-50/30 transition cursor-pointer flex gap-3 group ${isRead ? 'opacity-40 bg-white' : 'bg-blue-50/20'}`}>
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <div className="flex justify-between items-center mt-1"><span className="text-[9px] font-black uppercase text-gray-400">{item.type}</span>{item.date && <span className="text-[9px] text-gray-400">{item.date}</span>}</div>
                                            </div>
                                        </div>
                                    );
                                })}
             ...
             <div className="flex items-center gap-3 pl-6 border-l border-gray-200">
               <div className="text-right hidden md:block"><p className="text-sm font-bold text-gray-800 leading-tight">{currentUser.username}</p><p className="text-[10px] text-gray-500 uppercase font-bold">{currentUser.role}</p></div>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-sm p-6 animate-scale-in">
        ...
        <input type="password" placeholder="Nova Senha" className="w-full mb-3 px-4 py-2 border rounded-lg" value={newPass} onChange={e => setNewPass(e.target.value)} />
        <input type="password" placeholder="Repetir Senha" className="w-full mb-6 px-4 py-2 border rounded-lg" value={confirmPass} onChange={e => setConfirmPass(e.target.value)} />
        <div className="flex gap-3"><button onClick={onClose} className="flex-1 py-2 text-gray-600 font-medium">Cancelar</button><button onClick={handleSave} className="flex-1 py-2 bg-primary text-white rounded-lg font-bold">Salvar</button></div>
      </div>
    </div>
  );
};
